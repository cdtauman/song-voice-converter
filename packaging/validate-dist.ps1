...
foreach ($requiredDirectory in "torch", "audio_separator") {
    $runtimeDirectory = Join-Path $Distribution "_internal\$requiredDirectory"
    if (-not (Test-Path -LiteralPath $runtimeDirectory -PathType Container)) {
        throw "Missing engine runtime directory: $runtimeDirectory"
    }
}
...
try {
    ...
    if (-not $process.WaitForExit(120000)) { $process.Kill(); throw "Packaged engine smoke test timed out" }
