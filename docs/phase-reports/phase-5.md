...
- **430 בדיקות עוברות**, 6 מדולגות. ruff נקי · mypy נקי
...
**שני חסמים, שניהם חיצוניים לקוד. הקוד שמעל המתאם שלם ונבדק; הרשת המוטמעת נאמנה למקור.**

### א. נעילת תלויות ה-inference (re-lock) — חסם מימוש קונקרטי
ה-Spike של Phase 1 אימת "conversion" כעומס אופרטורים סינתטי
(`conv1d+transformer+convtranspose`), **לא** את RVC האמיתי. התלויות של inference אמיתי —
`faiss-cpu` (חיפוש ה-index) ו-`transformers` (טעינת HuBERT) — **אינן ב-`constraints.txt`**.
נעילתן דורשת הרצת ה-Spike מחדש על מחשב הפיתוח (`spike/run_spike.py` → `constraints.txt`
מעודכן), כולל אימות שהן חיות יחד עם `torch==2.13.0+xpu` הנעול. זהו בדיוק סוג החסם
שנעילת Phase 0 מתירה לפתוח בו את המטריצה. עד אז `RVCv2Backend.info()` מדווח `available=False`
עם סיבה בעברית — לא קורס.
...
`quality_probe.probe_quality_curve` מקבל `RVCv2Backend` בהזרקה, וברגע שחסמים (א)+(ב)
...
**מה צריך ממך:** re-lock של התלויות על מחשב הפיתוח, ואותם 5 שירים + 3 קולות יעד.
